...
阶段 G：后台/远程/报表（最后做）
 ...
 G3. 远程控制闸门（权限与审计）
 阶段 H:添加打印机，可以打印小票功能
    H1:在现金设备连通性测试里面新增一个选项卡为打印机，里面用于设置打印机的一些自定义信息，如票据内容抬头，默认为在票据左上角显示Welcome to ChuangLi store，然后还要有洗车店的地址，这个地址也是可以自定义的，默认为Room 2035, 2nd Floor, Chengyun Building, Shenzhou Industrial Zone, Bantian, Longgang District, Shenzhen
    H2:票据上面除了有抬头以外，还需显示以下内容，Invoice号码，这个号码需是唯一的，每次都不同
    H3:洗车时间，本次的洗车时间，日，月，年的格式输出
    H4:支付方式，是指对本次洗车费用的支付方式，如果是现金或硬币，就显示现金，如果是刷卡支付，就显示刷卡支付
    H5:洗车费用，是指本次洗车的费用，
    H6:洗车模式，是指本次洗车客户选择的洗车模式，是哪个，是基础版，还是标准版或高级版
    H7:Thank you have a nice day,显示在票据的右下角，
